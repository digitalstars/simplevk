...
if (isset($data->type) and $data->type == 'message_new') { //Проверяем, если это входящее сообщение
        ...
        if ($payload != null) { // если payload существует
            switch ($payload['animals']) { //Смотрим что в payload кнопках
                ...
                case 'Pink_salmon': //Если это Горбуша
                    $vk->sendMessageChat($id, "Держи свою горбушу!"); //отправляем сообщение
                    $vk->sendImageChat($id, "img/pink_salmon.jpg"); //отправляем картинку
                    break;
                case 'Goldfish': //Если это Золотая рыбка
                    $vk->sendMessageChat($id, "Она исполнит твои желания...");
                    $vk->sendImageChat($id, "img/goldfish.jpg");
                    break;
                case 'Plotva': //Если это Плотва
                    $vk->sendMessageChat($id, "Ой, похоже картинку перепутали)");
                    $vk->sendImageChat($id, "img/plotva.jpg");
                    break;
                default:
                    break;
            }
