...
* `sendDocMessage($id, $local_file_path, $title = null)` - отправка документа
    ...
    * `$local_file_path` - путь до файла
    * `$title` - название файла (если не указать, то останется локальное название)
* `sendVoice($id, $local_file_path)` - отправляет аудиофайл с расширениями |.mp3|.ogg|.pcm|.wav| как голосовое сообщение 
    * `$id` - id получателя
    * `$local_file_path` - путь до файла
...
* `setVoice($local_file_path)` - прикрепляет голосовое сообщение
