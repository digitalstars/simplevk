...
* `$vk = new vk_api('токен группы/пользователя', 'версия api')` - авторизауия через токен
...
* `sendOK()` - отправка текста 'ok', и сообщение клиенту, что-бы не ждал ответа скрипта (обход тайм-аута ВК)
* [sendButton](#Работа-с-клавиатурой)`($user_id, $message, $buttons = [], $one_time = False)` - отправка клавиатуры _(отправка только от имени группы)_
...
* `request($method, $params = [])` - Отправка запроса напрямую к api vk
    * `$method` - метод
...
* `sendImage($id, $local_file_path)` - Отправка изображения
    ...
    * `$local_file_path` - путь до сообщения
...
* `setTryCountResendFile($var)` - задаёт максимальное количество попыток загрузки файла
    * У вк бывает, с этим возникают некоторые баги и файл не загружается
    * `$var` - число попыток, по умолчанию 5, также можно настроить число по умолчанию в [файле конфигурации](#Файл-конфигурации)
* `setRequestIgnoreError($var)` - задаёт коды ошибок ВК, при которых сообщение об ошибке игнорируется и отправляется повторный запрос
...
#### Методы класса
Все те же, что и у [vk_api](#Класс-vk_api)
*******************
### Класс Auth
Класс нужен для кастомной авторизации в вк, можно задать множество параметров при авторизации и выбрать метод авторизации, также получить токен доступа или куки
#### Инициализация класса
...
может принимать значения: ['useragent' => 'пользовательский User-Agent', 'id_app' => 'id приложения для авторизации через приложение ВК'] 
...
#### Методы класса
* `auth()` - запускает процесс авторизации _(ТОЛЬКО ДЛЯ АВТОРИЗАЦИИ ЧЕРЕЗ ПРИЛОЖЕНИЕ ВК)_
    * После вызова метода происходит авторизация в ВК (получение авторизационнах кук), но не получение токена доступа.
...
* `getAccessToken($captcha_key = null, $captcha_sid = null)` - попытаться получить токен доступа, при успехи его и вернёт
    ...
    * `$captcha_sid` - сид каптчи, полученный при ошибки ([VkApiException](#Исключения-VkApiException), можно поймать через try catch)
...
* `addImage($images1, $images2,...)` - добавляет картинки во вложения
    ...
    * Если вызвать несколько, то они будут дополнять друг друга, а не заменять
...
******************
### Класс Message
Этот класс является удобным конструктором запросов для создания и отправки сообщений
#### Инициализация класса
...
* [setKeyboard](#Работа-с-клавиатурой)`($keyboard = [], $one_time = false)` - прикрепляет клавиатуру к сообщению (только от имени сообщества)
    * `$buttons` - массив клавиатуры
    * `$one_time` - исчезнет ли клавиатура после того, как пользователь ей воспользуется?
...
сама генерирует исключение, если что-то не так.
*******************
